...
from macad_gym.models import MLPNetwork
from macad_gym.utils import soft_update, average_gradients, onehot_from_logits, gumbel_softmax
from macad_gym.agent import TD3Agent
...
class MATD3(object):
    ...
    def __init__(self, agent_init_params, alg_types,
                 gamma=0.95, tau=0.01, lr=0.01, hidden_dim_1=512, hidden_dim_2=216,
    ...
    def update(self, sample, agent_i, parallel=False, logger=None):
        ...
        if self.alg_types[agent_i] == 'MATD3':
            if self.discrete_action: # one-hot encode action
                ...
                # print(torch.transpose(next_obs[0],0,1).shape)
                all_trgt_acs = [pi(torch.transpose(nobs,0,1)) for pi, nobs in zip(self.target_policies, next_obs)]
                # all_trgt_acs = [pi(nobs) for pi, nobs in zip(self.target_policies, next_obs)]
            trgt_vf_in = torch.cat((*next_obs, *all_trgt_acs), dim=1) # target_value_function input
            trgt_vf_in = torch.transpose(trgt_vf_in,0,1)
        ...
        if self.alg_types[agent_i] == 'MATD3':
            ...
            vf_in = torch.transpose(vf_in,0,1)
        else:  
        ...
        if self.discrete_action:
            ...
            curr_pol_out = curr_agent.policy(obs[agent_i].T)
        ...
        if self.alg_types[agent_i] == 'MATD3':
            ...
            for i, pi, ob in zip(range(self.nagents), self.policies, obs):
                if i == agent_i:
                    ...
                    all_pol_acs.append(onehot_from_logits(pi(ob.T)))
                else:
                    all_pol_acs.append(pi(ob.T))
            vf_in = torch.cat((*obs, *all_pol_acs), dim=1).T
        else:  # DDPG
    ...
    def to(self, device):
        """Move all agents to the specified device."""
