...
class GlobalRoutePlanner(object):
    ...
    """
    #####################################################################################
    def plot_graph(self, node_labels=None):
        # fig, ax = plt.subplots()
        # fig.set_tight_layout(False)
        plt.figure(figsize=(12,8))
        pos = {}                                                ### 그래프의 형태를 지정한다 - 현재는 각 kkey의 vertex 좌표에 노드 생성
        for kkey,vvalue in self._graph.nodes(data=True):
            x = vvalue['vertex'][0]
            y = vvalue['vertex'][1]
            pos.update({kkey:[x,-y]})                           ### 그래프에서 상하 반전해 줘야 보기 편해서 
            # plt.text(x,-y, f"x = {x:.8f} \n y = {y:.8f}", fontweight = 'normal' , va = 'center', ha = 'center')              ### 그래프에서 노드 밑에 좌표 보고 싶을때 
    ...
    def setup(self):
        ...
        ##################################################
        # self.plot_graph(self._graph)                  ### planner 대략적인 그림보고 싶을 때
        ##################################################
    ...
    def plan_route(self, origin, destination):
        ...
        threshold = math.radians(4.0)                                           ### 0.06981317007977318
        route = self.path_search(origin, destination)                           ### route:  [138, 139, 197, 340, 342]
        # print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
        # print("route: ", route)
        plan = []

        # Compare current edge and next edge to decide on action
        for i in range(len(route) - 2):                                         ### route에 origin과 destination이 포함되어 있기 때문에 -2
            current_edge = self._graph.edges[route[i], route[i + 1]]            ### edges 안에 
            next_edge = self._graph.edges[route[i + 1], route[i + 2]]
            #print("route[i,i+1]", route[:i+1])
    ...
    def path_search(self, origin, destination):
        ...
        route = nx.astar_path(                                          ### source 부터 target 까지 가장 짧은 경로의 list를 반환, weight 가 length이므로 길이 기준
    ...
    def localise(self, x, y):
        """
        This function finds the road segment closest to (x, y)                      ### 가장 가까운 (x,y)를 찾는 것
        ...
        for segment in self._topology:
            entryxy = segment['entry']              ### (x1, y1)
            exitxy = segment['exit']                ### (x2, y2)
            path = segment['path']
            for xp, yp in [entryxy] + path + [exitxy]:                
                new_distance = self.distance((xp, yp), (x, y))
                if new_distance < nearest[0]:
                    nearest = (new_distance, segment)                               ### 가장 가까운 것을 nearest로 지정
    ...
    def build_graph(self):
        ...
        for segment in self._topology:
            entryxy = segment['entry']                  # {entry: (x1,y1)}
            exitxy = segment['exit']                    
            path = segment['path']
            intersection = segment['intersection']
            for vertex in entryxy, exitxy:                  ### 2번 돈다 vertex가 entryxy 일때랑 exitxy일때 
