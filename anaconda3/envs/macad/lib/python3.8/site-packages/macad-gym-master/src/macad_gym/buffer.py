...
class ReplayBuffer(object):
    ...
    def push_hong(self, States, agent_actions, reward_dict, next_States, dones_dict):
        nentries = 1 # 단일 env              
        rewards = np.array(list(reward_dict.values()))
        dones = np.array(list(dones_dict.values())) 

        if self.curr_i + nentries > self.max_steps:
            rollover = self.max_steps - self.curr_i  # 이거 항상 1임
        ...
        for agent_i in range(self.num_agents):
            self.obs_buffs[agent_i][self.curr_i] = States[agent_i]
            self.ac_buffs[agent_i][self.curr_i] = agent_actions[agent_i]
            self.rew_buffs[agent_i][self.curr_i] = rewards[agent_i]
            self.next_obs_buffs[agent_i][self.curr_i] = next_States[agent_i]
            self.done_buffs[agent_i][self.curr_i] = dones[agent_i]
