...
from agents.navigation.local_planner import (  # noqa:E402, E501
    RoadOption,
)
...
class MultiCarlaEnv(*MultiAgentEnvBases):
    def __init__(self, configs=None):
        ...
        if self._discrete_actions:
            self.action_space = Dict(
                {
                    actor_id: Discrete(len(DISCRETE_ACTIONS)) # 9  actor_id : Discrete(9)
        ...
        self.pos_coor_map = MAP_TO_COORDS_MAPPING[self._map]           ### town03
    ...
    def _init_server(self):
        ...
        if IS_WINDOWS_PLATFORM:                                                         ### IS_WINDOWS_PLATFORM : False
        ...
        self._client.load_world(self._server_map)                                       ### _server_map = "/Game/Carla/Maps/Town03"
        self.world = self._client.get_world()
        world_settings = self.world.get_settings()
        world_settings.synchronous_mode = self._sync_server                             ### sync_server = True
        if self._sync_server:
            # Synchronous mode
            # try:planner
            # Available with CARLA version>=0.9.6
            # Set fixed_delta_seconds to have reliable physics between sim steps
            world_settings.fixed_delta_seconds = self._fixed_delta_seconds              ### _fixed_delta_seconds = 0.05 , 시뮬레이션 step 사이의 경과된 시간이 0.05로 고정되게 한다 
        self.world.apply_settings(world_settings)
        # Set up traffic manager
        self._traffic_manager = self._client.get_trafficmanager()
        self._traffic_manager.set_global_distance_to_leading_vehicle(2.5)               ### 차량 사이의 최소거리, 차량의 무게중심에서 중심사이의 거리로 계산, 단위: m
        self._traffic_manager.set_respawn_dormant_vehicles(True)                        ### True 일 경우 차량이 휴면상태가 되면 영웅차량(메인차량인듯?) 근처에서 다시 리스폰된다 
        self._traffic_manager.set_synchronous_mode(self._sync_server)                   ### traffic manager를 동기화 모드로 설정한다 멀티 클라이언트 일경우 TM-Server만 틱?할수 있다
        # Set the spectator/server view if rendering is enabled
        if self._render and self._env_config.get("spectator_loc"):                      
            spectator = self.world.get_spectator()
            spectator_loc = carla.Location(*self._env_config["spectator_loc"])          ### 메인 카메라의 위치를 지정
            d = 6.4
            angle = 160  # degrees
            a = math.radians(angle)                                                     ### a = 2.792526803190927
            location = (
                carla.Location(d * math.cos(a), d * math.sin(a), 2.0) + spectator_loc   ### (-6.014032773029814, 2.1889289172842807 ,2.0),  location: Vector3D(x=133.985962, y=70.188927, z=11.000000)
            )
            spectator.set_transform(
                carla.Transform(location, carla.Rotation(yaw=290 + angle, pitch=-90))   ### 원하는 위치로 spectator 를 set          
            )
    ...
    def reset(self):
        ...
        self._load_scenario(self._scenario_config)         ### _scenario_config = "SSUI3C_TOWN3",
        
        for retry in range(RETRIES_ON_ERROR):              ### RETRIES_ON_ERROR = 2
            try:
        ...
        for actor_id, actor_config in self._actor_configs.items():
            if self._done_dict.get(actor_id, True):
                ...
                info_dict[actor_id] = py_measurement
        ...
        return self._obs_dict, info_dict
    ...
    def _spawn_new_actor(self, actor_id):
        ...
        if actor_type == "pedestrian":
            ...
            if self.exclude_hard_vehicles:                                              ### exclude_hard_vehicles = false
                blueprints = list(
                    filter(
                        lambda x: not (
    ...
    def _reset(self, clean_world=True):
        ...
        for actor_id, actor_config in self._actor_configs.items():
            if self._done_dict.get(actor_id, True):
                ...
                if actor_config["log_images"]:                                ### log_images = False
                ...
                camera_type = self._actor_configs[actor_id]["camera_type"]    ### camera_type = rgb
                camera_pos = getattr(
                    self._actor_configs[actor_id], "camera_position", 0       ### _actor_configs 안에 "camera_position"가 없기 때문에 0 을 반환
                )
                ...
                # Manual Control
                if actor_config["manual_control"]:                           ### manual_control = False
        ...
        self._npc_vehicles, self._npc_pedestrians = apply_traffic(
            ...
            self._scenario_map.get("num_vehicles", 0),                               ### _scenario_map = SSUI3C_TOWN3
    ...
    def _encode_obs(self, actor_id, image, py_measurements):
        ...
        if prev_image is None:
            prev_image = image
        if self._framestack == 2:                                                   ### _framestack = 1
            # image = np.concatenate([prev_image, image], axis=2)
            image = np.concatenate([prev_image, image])
        # Structure the observation
        if not self._actor_configs[actor_id]["send_measurements"]:                  ### send_measurements = False
            return image
        obs = (
            image,
    ...
    def step(self, action_dict):
        ...
        try:
            ...
            self._done_dict["__all__"] = sum(self._done_dict.values()) >= len(
                self._actors
            ) # 이게 true 면 _done_dict 에 전부 all 이 들어감
    ...
    def _step(self, actor_id, action):
        ...
        if self._discrete_actions:
            action = DISCRETE_ACTIONS[int(action)]                    ### action : 3 -> [1.0, 1.0]
        assert len(action) == 2, "Invalid action {}".format(action)
        if self._squash_action_logits:                                ### _squash_action_logits = False
            forward = 2 * float(sigmoid(action[0]) - 0.5)
            throttle = float(np.clip(forward, 0, 1))
            brake = float(np.abs(np.clip(forward, -1, 0)))
            steer = 2 * float(sigmoid(action[1]) - 0.5)
        else:
            throttle = float(np.clip(action[0], 0, 0.6))              ### action[0]가 0보다 작으면 0이되고 0.6보다 크면 0.6이 된다 
            brake = float(np.abs(np.clip(action[0], -1, 0)))
            steer = float(np.clip(action[1], -1, 1))
        reverse = False
        hand_brake = False
        if self._verbose:                                                                  ### verbose = False
            print(
                "steer", steer, "throttle", throttle, "brake", brake, "reverse", reverse
            )
        config = self._actor_configs[actor_id]
        if config["manual_control"]:                             # manual_control = False
            ...
            agent_type = config.get("type", "vehicle")                           ### agent_type = vehicle_4W
    ...
    def _read_observation(self, actor_id):
        ...
        py_measurements = {
            ...
            "v_x" : self._actors[actor_id].get_velocity().x,
            "v_y" : self._actors[actor_id].get_velocity().y,
            "end_pos" : self._end_pos[actor_id]
        }
...
if __name__ == "__main__":
    ...
    for _ in range(2):
        ...
        while not done["__all__"]:
            ...
            # py_measurment 안에 있는 "next_command" 로 action_dict 를 주는게 get_next_actions 함수 
